...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (role === 'tenant' && allocatedRoomId) {
      const roomResult = await pool.query(
        'SELECT landlord_id, room_name FROM rooms WHERE room_id = $1 LIMIT 1',
        [allocatedRoomId]
      );
      if (roomResult.rows.length > 0) {
        ...
        if (room.landlord_id) {
          await pool.query(
            ...
            [room.landlord_id, 'New Tenant Registered', `Tenant "${username}" has been assigned to ${room.room_name}.`, 'tenant_added']
          );
        }
      }
    }
